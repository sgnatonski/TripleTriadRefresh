...
{
    public class JavaScriptProxyGenerator : IJavaScriptProxyGenerator
    {
        ...
        public JavaScriptProxyGenerator(IDependencyResolver resolver) : this(resolver.Resolve<IHubManager>(),
        resolver.Resolve<IJavaScriptMinifier>())
        {
        }

        public JavaScriptProxyGenerator(IHubManager manager, IJavaScriptMinifier javascriptMinifier)
        {
            _manager = manager;
        ...
        public string GenerateProxy(string serviceUrl)
        {
            ...
            foreach (var descriptor in _manager.GetHubs().OrderBy(h => h.Name))
            {
                ...
                GenerateType(hubs, descriptor, false);
                first = false;
            }
        ...
        private void GenerateType(StringBuilder sb, HubDescriptor descriptor, bool includeDocComments)
        {
            ...
            var methods = GetMethods(descriptor).Where(m => m.Hub != null);
        ...
        private IEnumerable<MethodDescriptor> GetMethods(HubDescriptor descriptor)
        {
            return from method in _manager.GetHubMethods(descriptor.Name)
                   ...
                   into overloads let oload = (from overload in overloads
        ...
        {
            return Commas(values.Where(v => v != null), v => v);
        }
